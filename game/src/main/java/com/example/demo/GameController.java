...
HttpSession session;
@GetMapping("/")
public String index() {session.invalidate();//答えを使ってSessionに格納
...
List<History> histories=(List <History>)session.getAttribute("histories");
 ...
 session.setAttribute("histories", histories);}//判定→回答履歴追加
 if
     (answer <number) {histories.add(new History (histories.size()+ 1, number, "もっと小さいです"));
 
 } else if
     (answer==number){histories.add(new History(histories.size() + 1,number, "正解です！"));
